...
<a href="https://github.com/npm/node-semver">node-semver</a>, which is bundled
...
<p>When a user installs your package, npm will emit warnings if packages specified in <code>peerDependencies</code> are not already installed. The <code>peerDependenciesMeta</code> field serves to provide npm more information on how your peer dependencies are to be used. Specifically, it allows peer dependencies to be marked as optional.</p>
<p>For example:</p>
<pre lang="json"><code>{
  "name": "tea-latte",
  "version": "1.3.5",
...
<p>Marking a peer dependency as optional ensures npm will not emit a warning if the <code>soy-milk</code> package is not installed on the host. This allows you to integrate and interact with a variety of host packages without requiring all of them to be installed.</p>
...
to find each <a href="../using-npm/workspaces.html">workspace</a> that needs to be symlinked to
the top level <code>node_modules</code> folder.</p>
...
<p>In the following example, all folders located inside the folder <code>./packages</code>
will be treated as workspaces as long as they have valid <code>package.json</code> files
inside them:</p>
<pre lang="json"><code>{
